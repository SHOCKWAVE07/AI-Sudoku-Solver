...
dataloader_ = data.DataLoader(train_set, batch_size=batch_size, pin_memory=True, shuffle=True)

# Validation data loader
dataloader_val_ = data.DataLoader(test_set, batch_size=batch_size, pin_memory=True, shuffle=True)
...
optimizer = optim.Adam(sudoku_solver.parameters(), lr=0.001, weight_decay=0.000)
...
for e in range(epochs):
    for i_batch, ts_ in enumerate(dataloader_):
        ...
        if i_batch % 2 == 0:
            print("Epoch " + str(e) + " batch " + str(i_batch) + ": " + str(ls.item()))
        sudoku_solver.eval()
        ...
        if i_batch % 20 == 0:
            with torch.no_grad():
                ...
                errors = test_fill.max(dim=2)[1] != sol_t[rows].max(dim=2)[1]
